...
function getConfig($configGroupKey,$config_key,$config_value,$config_group_count,$config_count)
    ...
    html = html + '<a href="#" onclick="deleteConfig(this);" id="delete-' + $configGroupKey + '-' + $config_group_count + '-' + $config_count + '-icon" title="Edit Property"><img src="https://s3.amazonaws.com/kinlane-productions/bw-icons/bw-delete-circle.png" width="20" align="right"  /></a>';           
	...
	$thisCount = 0;
 	$.each($MasterConfig[$configGroupKey], function(paramKey, paramValue) {
 		...
 		if($thisCount != $config_count)
			...
			$rebuildConfigArray = [];	  
			$rebuildConfigArray[$config_key] = $config_value;
